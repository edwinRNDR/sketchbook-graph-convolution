...
class SparseMatrix(
    ...
    operator fun times(scalar: Double): SparseMatrix {
        ...
        return SparseMatrix(rows, cols, newValues, columnIndices.clone(), rowPointers.clone())
    }
    ...
    fun isSymmetric(tolerance: Double = 1e-10): Boolean {
        ...
        for (i in 0 until rows) {
            for (j in 0 until i) { // Only check lower triangle
                if (Math.abs(dense[i, j] - dense[j, i]) > tolerance) {
                    return false
                }
            }
        }
    ...
    fun transpose(): SparseMatrix {
        ...
        val colPositions = newRowPointers.clone()
